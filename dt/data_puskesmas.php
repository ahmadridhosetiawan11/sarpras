...
<head>
  ...
  <tbody>
    ...
    <tr>
      <td><h5>Perihal</h5></td>
      <td>:</td>
      <td><h5>Seluruh Data Puskesmas</h5></td>
    </tr>
  </tbody>
</table>
	...
	<div class="container">
	<h2 class="text-center">DAFTAR PUSKESMAS WILAYAH PROV-KALSEL</h2><br>
	<table class="table table-bordered table-hover table-sm" style="margin: 0 auto">
      ...
      if (isset($_POST['cetak'])) {
        ...
        $result = mysqli_query($koneksi, "SELECT * FROM puskesmas INNER JOIN daerah ON puskesmas.id_d=daerah.id_d INNER JOIN pegawai ON puskesmas.id_pegawai=pegawai.id_pegawai WHERE puskesmas.tgl LIKE '%$bulan%' AND puskesmas.tgl LIKE '%$tahun%' ORDER BY tgl ASC");

      }else{
        $result = mysqli_query($koneksi, "SELECT * FROM puskesmas INNER JOIN daerah ON puskesmas.id_d=daerah.id_d INNER JOIN pegawai ON puskesmas.id_pegawai=pegawai.id_pegawai");
      }
...
$no = 1;
            ...
            <td>Kelurahan <?php echo $data['desa'] ?>, Kecamatan <?php echo $data['kec'] ?> RT <?php echo $data['rt'] ?> RW <?php echo $data['rw'] ?> Kota <?php echo $data['kota'] ?></td>
