...
<head>
  ...
  <title>Data Aset Tidak Layak / Layak</title>
  ...
  <tbody>
    ...
    <tr>
      <td><h5>Perihal</h5></td>
      <td>:</td>
      <td><h5>Seluruh Data Aset Tidak Layak / Layak</h5></td>
    </tr>
  </tbody>
</table>
...
<div class="tujuan">
  ...
  <div class="container">
  <h2 class="text-center">DAFTAR Aset Tidak Layak / Layak </h2>
  ...
  <table class="table table-bordered table-hover table-sm" style="margin: 0 auto">
      ...
      if (isset($_POST['cetak'])) {
        ...
        $ttd = $_REQUEST['ttd'];
        $result = mysqli_query($koneksi, "SELECT * FROM aset INNER JOIN sarpras ON aset.id_sarpras=sarpras.id_sarpras WHERE aset.tgl_aset LIKE '%$bulan%' AND aset.tgl_aset LIKE '%$tahun%' ORDER BY tgl_aset ASC");
        $result1 = mysqli_query($koneksi, "SELECT * FROM pegawai WHERE pegawai.id_pegawai = '$ttd'");

      }else if (isset($_POST['cetak2'])) {
        $status_aset = $_REQUEST['status_aset'];
        $ttd = $_REQUEST['ttd'];
        $result = mysqli_query($koneksi, "SELECT * FROM aset INNER JOIN sarpras ON aset.id_sarpras=sarpras.id_sarpras WHERE status_aset = '$status_aset'");
        $result1 = mysqli_query($koneksi, "SELECT * FROM pegawai WHERE pegawai.id_pegawai = '$ttd'"); 
      }else if (isset($_POST['cetak3'])) {
        $ttd = $_REQUEST['ttd'];
        $result = mysqli_query($koneksi, "SELECT * FROM aset INNER JOIN sarpras ON aset.id_sarpras=sarpras.id_sarpras");
        $result1 = mysqli_query($koneksi, "SELECT * FROM pegawai WHERE pegawai.id_pegawai = '$ttd'"); 
      }else{
        $result = mysqli_query($koneksi, "SELECT * FROM aset INNER JOIN sarpras ON aset.id_sarpras=sarpras.id_sarpras");
      }
