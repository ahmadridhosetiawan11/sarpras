...
<?php 
  if (isset($_POST['cetak'])) {
        ...
        $ttd = $_REQUEST['ttd'];
        $result = mysqli_query($koneksi, "SELECT * FROM perbaikan INNER JOIN sarpras ON perbaikan.id_sarpras=sarpras.id_sarpras WHERE perbaikan.tgl_pbk LIKE '%$bulan%' AND perbaikan.tgl_pbk LIKE '%$tahun%'");
        $result1 = mysqli_query($koneksi, "SELECT * FROM pegawai WHERE pegawai.id_pegawai = '$ttd'"); 

      }else if (isset($_POST['cetak2'])) {
        $ttd = $_REQUEST['ttd'];
        $result = mysqli_query($koneksi, "SELECT * FROM perbaikan INNER JOIN sarpras ON perbaikan.id_sarpras=sarpras.id_sarpras");
      ...
      if(mysqli_num_rows($result)==0) {
        echo '<script>alert("Data Tidak Ada !"); window.location.href="../adimin/cperbaikan";</script>';
  ...
  $pdf->Cell(40,10,'Nama Sarpras',1,0,'C');
  $pdf->Cell(52,10,'Tgl Perbaikan $ Tgl Selesai',1,0,'C');
  $pdf->Cell(14,10,'Jumlah',1,0,'C');
  $pdf->Cell(45,10,'Keterangan',1,0,'C');
  $pdf->Cell(37,10,'Biaya Perbaikan',1,0,'C');
  $pdf->Cell(40,10,'Kondisi Perbaikan',1,0,'C');
  $pdf->Cell(44,10,'Status',1,0,'C');
  $pdf->Cell(0,10,'',0,1);
  ?>
  <?php
  ...
  <?php
    ...
    $pdf->Cell(40,10,$lihat['nama_s'],1,0,'C');
    $pdf->Cell(52,10,tgl_indo($lihat['tgl_pbk']).' / '.tgl_indo($lihat['tgl_sel']),1,0,'C');
    $pdf->Cell(14,10,$lihat['jml_pbk'],1,0,'C');
    $pdf->Cell(45,10,$lihat['ket_pbk'],1,0,'C');
    $pdf->Cell(37,10,rupiah($lihat['biaya_pbk']),1,0,'C');
    $pdf->Cell(40,10,$lihat['kondisi_pbk'],1,0,'C');
    $pdf->Cell(44,10,$lihat['status_pbk'],1,0,'C');
    $pdf->Cell(5,10,'',0,1);
    $nomor++;
  }
  ?>
  ...
  $pdf->Output("Data Perbaikan Sarpras.pdf","I"); 
?>