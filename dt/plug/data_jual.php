...
<?php 
  if (isset($_POST['cetak'])) {
        ...
        $ttd = $_REQUEST['ttd'];
        $result = mysqli_query($koneksi, "SELECT * FROM jual INNER JOIN sarpras ON jual.id_sarpras=sarpras.id_sarpras WHERE jual.tgl_j LIKE '%$bulan%' AND jual.tgl_j LIKE '%$tahun%' ORDER BY tgl_j ASC");

        $result1 = mysqli_query($koneksi, "SELECT * FROM pegawai WHERE pegawai.id_pegawai = '$ttd'"); 
      }else if (isset($_POST['cetak2'])) {
        $ttd = $_REQUEST['ttd'];
        $result = mysqli_query($koneksi, "SELECT * FROM jual INNER JOIN sarpras ON jual.id_sarpras=sarpras.id_sarpras");
      ...
      if(mysqli_num_rows($result)==0) {
        echo '<script>alert("Data Tidak Ada !"); window.location.href="../adimin/cjual.php";</script>';
  ...
  $pdf->SetFont('Times','B',14);
  $pdf->Cell(0,4,'DAFTAR DATA PENJUALAN BARANG / SARANA',0,1,'C');
  ...
  $pdf->Cell(45,10,'Tgl Jual',1,0,'C');
  $pdf->Cell(55,10,'Jumlah Jual',1,0,'C');
  $pdf->Cell(55,10,'Harga Jual',1,0,'C');
  $pdf->Cell(0,10,'',0,1);
  ?>
  <?php
  ...
  <?php
    ...
    $pdf->Cell(45,10,tgl_indo($lihat['tgl_j']),1,0,'C');
    $pdf->Cell(55,10,$lihat['jml_j'],1,0,'C');
    $pdf->Cell(55,10,rupiah($lihat['harga_j']),1,0,'C');
    $pdf->Cell(5,10,'',0,1);
    $nomor++;
  }
  ?>
  ...
  $pdf->Output("Data Penjualan Sarpras.pdf","I"); 
?>